...
export function applyMessage(state: JanitorState, message: JanitorMessage): void {
  ...
  if (message.candidates !== undefined) state.candidates = [...message.candidates];
...
export async function cleanup(state: JanitorState, deps: CleanupDeps): Promise<void> {
  ...
  const ids = [...new Set([...(state.hookId === null ? [] : [state.hookId]), ...state.candidates])];
...
async function main(): Promise<void> {
  ...
  const log = (event: string, fields: Record<string, unknown> = {}): void => {
    process.stderr.write(`${JSON.stringify({ ts: new Date().toISOString(), component: 'janitor', event, ...fields })}\n`);
  };
